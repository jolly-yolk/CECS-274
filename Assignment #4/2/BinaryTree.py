...
class BinaryTree:
    class Node:
        def __init__(self, x, v = None) : #x = object
    ...
    def height2(self, u):
        if (self.r == None):
            return 1
        if (self.depth(u) != 0) and (u.right == u.left == None):
            return 1
        
        nodes_childern = self._size(u) - 1
        h = 0
        n = 1
    ...
    def is_balanced(self):
        return self._is_balanced(self.r)
    
    def _is_balanced(self, u):
        
        if (u == None):
            return True
        
        if (abs(self._height(u.left) - self._height(u.right)) <= 1) and (self._is_balanced(u.left) == True) and (self._is_balanced(u.right) == True):
                return True
            
        return False
        
        '''
        sx = self._size(u.left)
        sy = self._size(u.right)
        if (sx != sy):
            if sx > sy:
                sy += 1
            else:
        ...
        if (sx == sy):
                if (u.left != None):
                    self.is_balanced(u.left)
                if (u.right != None):
                    self.is_balanced(u.right)
        else:
            return False
        '''
